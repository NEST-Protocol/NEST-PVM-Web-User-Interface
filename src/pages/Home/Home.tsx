...
const Home: FC = () => {
  ...
  const { data: kols } = useSWR(
    "https://api.nestfi.net/api/users/kol/count",
    (url) =>
      fetch(url)
        .then((res) => res.json())
        .then((data) => data.value)
  );
  const { data: destoryData } = useSWR(
    `https://api.nestfi.net/api/dashboard/destory?chainId=${
      chainsData.chainId === 534353 ? 534353 : 56
    }`,
    (url: string) =>
      fetch(url)
        .then((res) => res.json())
  ...
  const { data: txInfo } = useSWR(
    `https://api.nestfi.net/api/dashboard/txVolume?chainId=${
      chainsData.chainId === 534353 ? 534353 : 56
    }`,
    (url: string) =>
      fetch(url)
              ...
              {[
                {
                  ...
                  value: txInfo
                    ? Number(txInfo?.totalVolume?.toFixed(2))?.toLocaleString()
                    : "-",
                ...
                },
                {
                  ...
                  value: destoryData
                    ? Number(destoryData?.totalDestroy?.toFixed(2))
                        ?.toLocaleString()
                        .replace("-", "") || 0
                    : "-",
                  chainId: [56, 97, 534353],
                ...
                .filter((item) =>
                  item.chainId.includes(chainsData.chainId ?? 56)
                )
                .map((item, index) => (
              ...
              {[
                {
                  title: t`Total Trading Volume (NEST)`,
                  value: txInfo
                    ? txInfo?.totalVolume?.toFixed(2)?.toLocaleString()
                    : "-",
      ...
      >
        {width >= WidthType.lg && chainsData.chainId !== 534353 && (
          ...
          >
            <Stack
              ...
              alignItems={"center"}
              py={["40px", "40px", "40px", "40px", "40px", "44px"]}
              px={"40px"}
              sx={(theme) => ({
                background: theme.normal.bg1,
              })}
            >
              ...
              >
                <Trans>
                  Join NESTFi affiliate program and get commission on your
                  referrals.
                </Trans>
              </Box>
              <MainButton
        ...
        >
          ...
          >
            ...
            <Stack
              direction={width <= WidthType.md ? "column" : "row"}
              spacing={"24px"}
            >
              {[
                {
        ...
        >
          ...
          >
            ...
            <Stack
              spacing={"12px"}
              alignItems={width <= WidthType.lg ? "center" : "start"}
            >
              <Box
                sx={(theme) => ({
                  fontSize: ["24px", "32px"],
