...
You can follow the right person on NESTFi, here is my refer link: ${link}`;
  ...
  const spString = useMemo(() => {
    if (props.value.sp && props.value.sp !== 0) {
      return props.value.sp?.toLocaleString("en-US", {
  ...
  const slString = useMemo(() => {
    if (props.value.sl && props.value.sl !== 0) {
      return props.value.sl?.toLocaleString("en-US", {
        maximumFractionDigits: 2,
      });
    } else {
      return String().placeHolder;
        ...
        <BaseModal>
          ...
          >
            ...
            {dataUrl ? (
              ...
              <Stack
                minHeight={"400px"}
                height={"calc(min(100vw, 450px) * 1.4622)"}
                alignItems={"center"}
                spacing={"18px"}
